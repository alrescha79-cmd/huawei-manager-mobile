...
export function WebViewLogin({
  ...
  const handleMessage = useCallback(
    (event: { nativeEvent: { data: string; sourceUrl?: string } }) => {
      try {
        const sourceUrl = event.nativeEvent.sourceUrl || '';
        if (sourceUrl && !sourceUrl.includes(`http://${modemIp}/`)) {
          return;
        }
        ...
        if (message.type === 'LOGIN_SUCCESS') {
          ...
          ThemedAlertHelper.alert(
            t('webViewLogin.loginFailed'),
            message.message || t('webViewLogin.checkCredentials')
          );
        }
      } catch {}
    },
    [onLoginSuccess, modemIp]
  );
  ...
  const handleNavigationChange = useCallback(
    (navState: WebViewNavigation) => {
      ...
      const modemHost = modemIp.split(':')[0];
  ...
  const handleShouldStartLoad = useCallback(
    (request: { url: string }) => {
      const urlHost = request.url.match(/^https?:\/\/([^/]+)/)?.[1];
      const modemHost = modemIp.split(':')[0];
      return !urlHost || urlHost === modemHost;
    },
    [modemIp]
  );
  ...
  const handleClose = () => {
    if (!loginDetected) {
      ThemedAlertHelper.alert(t('webViewLogin.cancelLogin'), t('webViewLogin.cancelConfirm'), [
      ...
      {hidden ? (
            ...
            {isTimedOut ? (
              <>
                <Text
                  style={[
                    typography.headline,
                    { color: colors.error, marginBottom: spacing.sm, textAlign: 'center' },
                  ]}
                >
                ...
                <Text
                  style={[
                    typography.caption1,
                    { color: colors.textSecondary, marginBottom: spacing.lg, textAlign: 'center' },
                  ]}
                >
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
          ...
          >
            ...
            <Text
              style={[typography.headline, { color: colors.text, flex: 1, textAlign: 'center' }]}
            >
          ...
          <View style={[styles.instructions, { backgroundColor: colors.card }]}>
            <Text
              style={[typography.caption1, { color: colors.textSecondary, textAlign: 'center' }]}
            >
          ...
          <WebView
            ...
            onError={() => {
              console.error('[WebViewLogin] WebView load error');
              ThemedAlertHelper.alert(
