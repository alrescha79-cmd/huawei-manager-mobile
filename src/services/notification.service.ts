...
export async function checkIPChangeNotification(
  ...
  if (now - lastIpChangeNotifyTimestamp < NOTIFICATION_COOLDOWN_MS) {
    await AsyncStorage.setItem(LAST_SESSION_DURATION_KEY, currentSessionDuration.toString());
    return null;
  }
  ...
  if (currentSessionDuration < previousDuration && previousDuration > 60) {
    ...
    if (currentMinutes <= 0) durationText = '0';
      ...
      durationText =
        remainingMinutes > 0 ? `${currentHours}h ${remainingMinutes}m` : `${currentHours}h`;
    } else {
      durationText = `${currentMinutes}m`;
    }

    await sendLocalNotification(translations.title, translations.body(durationText), 'ip-change');
...
export async function checkNewSMSNotification(
  ...
  if (currentUnreadCount > lastCount && currentUnreadCount > 0) {
    ...
    await sendLocalNotification(translations.title, translations.body(newCount), 'sms-alerts', {
      route: '/sms',
    });
  } else {
    await AsyncStorage.setItem(LAST_SMS_COUNT_KEY, currentUnreadCount.toString());
...
export async function sendDebugModeReminder(translations: {
  ...
  await sendLocalNotification(translations.title, translations.body, 'debug-reminder', {
    route: '/(tabs)/settings',
  });
}
