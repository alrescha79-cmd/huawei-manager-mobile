...
export class DirectAuthService {
  ...
  private xmlRequest(method: 'GET' | 'POST', endpoint: string, body?: string): Promise<string> {
  ...
  private async initSession(): Promise<void> {
    ...
    console.log('[DirectAuth] Session established');
    console.log('[DirectAuth] Token acquired');
  }
  ...
  private async refreshToken(): Promise<void> {
    ...
    if (token) {
      ...
      console.log('[DirectAuth] Refreshed token');
    }
  }
  ...
  async login(password: string, username: string = 'admin'): Promise<DirectAuthSession> {
    ...
    try {
      ...
      const challengeXml = `<?xml version="1.0" encoding="UTF-8"?><request><username>${escapeXml(username)}</username><firstnonce>${clientNonce}</firstnonce><mode>1</mode></request>`;
      const challengeData = await this.xmlRequest(
        'POST',
      ...
      const clientProof = await this.calculateScramProof(
        password,
        clientNonce,
        serverNonce,
        salt,
      ...
      console.log('[DirectAuth] Client proof calculated');
