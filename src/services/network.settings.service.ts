...
import { APNProfile, EthernetSettings, EthernetStatus, DHCPSettings } from '@/types';
      ...
      if (profilesXML) {
        profilesXML.forEach((profileXML, index) => {
          const profile = {
            ...
            name:
              parseXMLValue(profileXML, 'Name') || parseXMLValue(profileXML, 'ProfileName') || '',
            apn: parseXMLValue(profileXML, 'ApnName') || parseXMLValue(profileXML, 'APN') || '',
            username:
              parseXMLValue(profileXML, 'Username') || parseXMLValue(profileXML, 'UserName') || '',
            password: parseXMLValue(profileXML, 'Password') || '',
            authType: this.parseAuthType(
              parseXMLValue(profileXML, 'AuthMode') || parseXMLValue(profileXML, 'AuthType')
            ),
            ipType: this.parseIpType(
              parseXMLValue(profileXML, 'IpType') ||
                parseXMLValue(profileXML, 'IPType') ||
                parseXMLValue(profileXML, 'PdpType')
            ),
            isDefault:
              parseXMLValue(profileXML, 'IsDefault') === '1' ||
...
<ApnName>${escapeXml(profile.apn)}</ApnName>
<DialupNum>*99#</DialupNum>
<Username>${escapeXml(profile.username || '')}</Username>
<Password>${escapeXml(profile.password || '')}</Password>
...
<Modify>2</Modify>
<Profile>
<Index>${escapeXml(profile.id)}</Index>
<IsValid>1</IsValid>
<Name>${escapeXml(profile.name)}</Name>
<ApnIsStatic>1</ApnIsStatic>
<ApnName>${escapeXml(profile.apn)}</ApnName>
...
<Modify>0</Modify>
  ...
  async setDHCPSettings(settings: Partial<DHCPSettings>): Promise<boolean> {
    try {
      ...
      const data = `<?xml version="1.0" encoding="UTF-8"?>
<request>
<DhcpIPAddress>${escapeXml(merged.dhcpIPAddress || '')}</DhcpIPAddress>
<DhcpLanNetmask>${escapeXml(merged.dhcpLanNetmask || '')}</DhcpLanNetmask>
<DhcpStatus>${merged.dhcpStatus ? '1' : '0'}</DhcpStatus>
<DhcpStartIPAddress>${escapeXml(merged.dhcpStartIPAddress || '')}</DhcpStartIPAddress>
<DhcpEndIPAddress>${escapeXml(merged.dhcpEndIPAddress || '')}</DhcpEndIPAddress>
<DhcpLeaseTime>${merged.dhcpLeaseTime}</DhcpLeaseTime>
<DnsStatus>${merged.dnsStatus ? '1' : '0'}</DnsStatus>
<PrimaryDns>${escapeXml(merged.primaryDns || '')}</PrimaryDns>
<SecondaryDns>${escapeXml(merged.secondaryDns || '')}</SecondaryDns>
</request>`;
  ...
  async getEthernetSettings(): Promise<EthernetSettings> {
    try {
      ...
      const modeValue =
        parseXMLValue(response, 'CradleMode') ||
  ...
  async getEthernetStatus(): Promise<EthernetStatus> {
    try {
      ...
      return {
        connected:
          parseXMLValue(response, 'CradleStatus') === '1' ||
          parseXMLValue(response, 'Status') === '1' ||
          parseXMLValue(response, 'Connected') === '1',
        ipAddress:
          parseXMLValue(response, 'CradleIPAddress') || parseXMLValue(response, 'IPAddress') || '',
        gateway:
          parseXMLValue(response, 'CradleGateway') || parseXMLValue(response, 'Gateway') || '',
        netmask:
          parseXMLValue(response, 'CradleNetMask') ||
          parseXMLValue(response, 'NetMask') ||
          parseXMLValue(response, 'SubnetMask') ||
          '',
        dns1:
          parseXMLValue(response, 'CradlePrimaryDNS') ||
          parseXMLValue(response, 'PrimaryDNS') ||
          parseXMLValue(response, 'DNS1') ||
          '',
        dns2:
          parseXMLValue(response, 'CradleSecondaryDNS') ||
          parseXMLValue(response, 'SecondaryDNS') ||
          parseXMLValue(response, 'DNS2') ||
          '',
        macAddress:
          parseXMLValue(response, 'CradleMACAddress') ||
