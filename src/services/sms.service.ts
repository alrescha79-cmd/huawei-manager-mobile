...
const MOCK_INBOX_MESSAGES: SMSMessage[] = [
  ...
  },
  {
    ...
    phone: 'Indosat',
    content:
      'Promo spesial IM3! Dapatkan kuota 50GB hanya Rp50.000. Buruan aktifkan sekarang di *123*888#. Berlaku hingga 31 Januari 2026. Info lengkap: indosat.com/promo',
  ...
  },
  {
    ...
    phone: '081234567890',
    content:
      'Halo, ini dari kantor. Besok ada meeting jam 9 pagi ya. Jangan lupa bawa laptop dan dokumen presentasi Q4. Terima kasih.',
  ...
  },
  {
    ...
    phone: 'BANK BCA',
    content:
      'Transaksi berhasil. Transfer Rp500.000 ke rek 1234567890 a.n JOHN DOE. Saldo: Rp2.500.000. Jika bukan Anda, hubungi 1500888.',
  ...
  },
  {
    ...
    phone: 'Smartfren',
    content:
      'Selamat datang di jaringan Smartfren. Nikmati pengalaman 4G LTE terbaik dengan kecepatan hingga 100Mbps. Aktifkan paket unlimited sekarang di *123# atau melalui aplikasi MySmartfren.',
...
let mockMessages = [...MOCK_INBOX_MESSAGES, ...MOCK_SENT_MESSAGES];

let mockCount: SMSCount = {
  localUnread: 2,
  localInbox: 10,
  localOutbox: 3,
  localDraft: 0,
...
export class SMSService {
  ...
  async getSMSList(
    ...
    if (useMockSMSData) {
      const filtered = mockMessages.filter((msg) => msg.boxType === boxType);
  ...
  async deleteSMS(index: string): Promise<boolean> {
    if (useMockSMSData) {
      const deleted = mockMessages.find((m) => m.index === index);
      mockMessages = mockMessages.filter((m) => m.index !== index);
      if (deleted?.smstat === '0') {
        mockCount.localUnread = Math.max(0, mockCount.localUnread - 1);
      }
      if (deleted?.boxType === 1) {
        mockCount.localInbox = Math.max(0, mockCount.localInbox - 1);
      } else if (deleted?.boxType === 2) {
        mockCount.localOutbox = Math.max(0, mockCount.localOutbox - 1);
      }
      return true;
    }
  ...
  async markAsRead(index: string): Promise<boolean> {
    if (useMockSMSData) {
      const msg = mockMessages.find((m) => m.index === index);
      if (msg && msg.smstat === '0') {
        msg.smstat = '1';
        mockCount.localUnread = Math.max(0, mockCount.localUnread - 1);
      }
