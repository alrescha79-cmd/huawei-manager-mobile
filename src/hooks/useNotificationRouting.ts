...
export function useNotificationRouting(authReady: boolean) {
  ...
  const handleNotificationResponse = (response: Notifications.NotificationResponse) => {
    ...
    const data = rawData as
      | { route?: string; url?: string; type?: string; body?: { route?: string; url?: string } }
      | undefined;
    ...
    if (route && !ROUTE_ALLOWLIST.has(route)) route = undefined;
    if (url && !isSafeExternalUrl(url)) url = undefined;

    const openWebView = (targetUrl: string) => {
      router.push({
        pathname: '/webview',
        params: { url: targetUrl, title: notificationTitle || 'Link' },
      });
    };
    ...
    try {
      ...
      fcmUnsubscribe = messaging().onMessage(async (remoteMessage: any) => {
        ...
        if (remoteMessage?.data && !remoteMessage?.notification) {
          ...
          if (title && body) {
            const safeRoute =
              typeof route === 'string' && ROUTE_ALLOWLIST.has(route) ? route : undefined;
            const safeUrl = typeof url === 'string' && isSafeExternalUrl(url) ? url : undefined;
            await Notifications.scheduleNotificationAsync({
              content: {
                title: String(title),
                body: String(body),
                data: { route: safeRoute, url: safeUrl },
                sound: true,
              },
              trigger: null,
            });
