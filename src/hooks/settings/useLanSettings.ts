...
export function useLanSettings({ t }: UseLanSettingsProps) {
  ...
  const [networkSettingsService, setNetworkSettingsService] =
    useState<NetworkSettingsService | null>(null);

  const generationRef = useRef(0);
  const isStale = (gen: number) => gen !== generationRef.current;

  const reloadApnTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    return () => {
      if (reloadApnTimerRef.current) clearTimeout(reloadApnTimerRef.current);
    };
  }, []);
  ...
  useEffect(() => {
    if (credentials?.modemIp) {
      ...
      loadEthernet(service, gen);
      loadDhcp(service, gen);
      loadApn(service, gen);
    }
  }, [credentials]);

  const loadEthernet = async (service: NetworkSettingsService, gen?: number) => {
  ...
  const loadDhcp = async (service: NetworkSettingsService, gen?: number) => {
    if (gen !== undefined && isStale(gen)) return;
    try {
  ...
  const loadApn = async (service: NetworkSettingsService, gen?: number) => {
    if (gen !== undefined && isStale(gen)) return;
    try {
  ...
  const handleSaveApnProfile = async (profileData: any) => {
    ...
    try {
      ...
      if (reloadApnTimerRef.current) clearTimeout(reloadApnTimerRef.current);
      reloadApnTimerRef.current = setTimeout(() => loadApn(networkSettingsService), 2000);
    } catch {
      ThemedAlertHelper.alert(t('common.error'), t('common.error'));
    } finally {
