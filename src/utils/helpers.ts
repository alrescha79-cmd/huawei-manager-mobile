...
const SAFE_URL_HOSTS = ['github.com', 'cakson.my.id'];
...
/**
 * Allow only http(s) URLs pointing at known first-party hosts or LAN IPs.
 * Blocks javascript:, file:, data: and any external domain.
 */
export const isSafeExternalUrl = (url: string): boolean => {
  const match = url.match(/^https?:\/\/([^/:]+)(?::\d+)?(?:\/|$)/i);
  if (!match) return false;
  const host = match[1].toLowerCase();
  return SAFE_URL_HOSTS.some((h) => host === h || host.endsWith(`.${h}`)) || isPrivateHost(host);
};

/**
 * Escape user-supplied values before interpolating them into Huawei XML
 * request bodies. Prevents XML injection.
 */
...
export const estimateLteBand = (
  cellId: number | string | undefined,
...
export const estimateLteBandwidth = (
  band: string | undefined
): { dl: string; ul: string } | undefined => {
